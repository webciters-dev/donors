...
// GET /api/boardMembers/:id - Get specific board member (Admin only) - SIMPLIFIED
router.get('/:id', requireAuth, onlyRoles('ADMIN'), async (req, res) => {
  try {
    const { id } = req.params;

    const boardMember = await prisma.boardMember.findUnique({
      where: { id: id } // Don't parse as int, use string ID
    });
...
router.delete('/:id', requireAuth, onlyRoles('ADMIN'), async (req, res) => {
  try {
    ...
    // For now, just do hard delete since we don't have interview system yet
